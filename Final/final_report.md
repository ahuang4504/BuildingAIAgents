...
- `qa/` — the blocking QA loop, with a terminal renderer for `plain_english` and `true_false` questions and an optional Gradio renderer seam for `faded_example` coding questions.
...
**Control flow.** When Claude Code proposes a mutation, the hook normalizes the call into a `ChangeProposal`, builds an aggregated context packet, and hands both plus the full competence model to the gate. The gate is a single model call (via OpenRouter, wrapped in a thin LangChain structured-output adapter) that returns a `GateDecision` of `allow` or `block`. On `allow`, the hook returns immediately and the original tool call resumes. On `block`, the hook stays suspended, the QA loop runs in the same process against the user's terminal, and only after the QA loop resolves does the hook return its final `allow` to Claude Code.
...
**Deliberate non-choices.** No LangGraph. No agent framework. No multi-turn orchestration object. The hook process is born, runs synchronously, and dies. All state that needs to survive lives on disk.
...
> "The first time it blocked me was on a regex change and I genuinely did not know what `(?<=...)` meant. I had been about to ship it. That was the moment I stopped rolling my eyes at the project."

> "Loved the true/false ones. The faded-example ones felt like a take-home interview in the middle of my flow. I'd want a 'skip, I accept the debt' button more obviously surfaced."

> "It caught me on async stuff twice in a row and the second question actually built on the first. That felt like a tutor, not a quiz."

> "It blocked a one-line rename. That was annoying. I want it to be smarter about how trivial a change is."
...
────────────────────────────────────────────────────────────────
VibeCheck: this change needs a quick check before it lands.

The diff adds:
    logger = logging.getLogger(__name__)
at module scope, then calls logger.info(...) inside `add()`.

Q: Why is the logger created at module scope (top of the file)
...
Q: Close. Loggers in Python are also looked up by name from a
...
**Synchronous blocking in the hook works, and it's the right shape.** Most of the early design risk was around whether suspending a Claude Code tool call mid-flight to run a terminal Q&A would feel terrible. It does not. Users reported the pause feels like a code review checkpoint, not an interruption — provided the gate fires on changes that actually deserve scrutiny.
...
The MVP is in scope, file-based, debuggable, and useful enough that several of the qualitative-test participants asked to keep it installed. That is a stronger signal than the numeric ratings.
